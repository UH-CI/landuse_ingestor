...
def createMetadata(j,i,x,y,dataset_name,token):
  ...
  query = {'value.name':dataset_name,'value.x':i,'value.y':j}
  json_query = json.dumps(query)
  com = '~/apps/cli/bin/metadata-list -z '+token+' -v -Q '+"'"+json_query+"'"+' > /tmp/res_'+str(i)+'_'+str(j)+'.json'
  comm ="~/apps/cli/bin/metadata-list -z %s -v -Q %s > /tmp/res_%s_%s.json"% (token,json_query,i,j)
  #call(comm)
  call('~/apps/cli/bin/metadata-list','-z', token)#, '-v', '-Q', json_query,' > /tmp/res_{str(i)}_{str(j)+}.json')
  #call("~/apps/cli/bin/metadata-list -z "+token+" -v -Q \"{\'value.name\':\'"+dataset_name+"\',\'value.x\':"+str(i)+",\'value.y\':"+str(j)+"}\" > /tmp/res_"+str(i)+"_"+str(j)+".json")
  with open("/tmp/res_"+str(i)+"_"+str(j)+".json") as jf:
    data = json.load(jf) 
  print(str(data['value']['recharge_scenario0']))
  print(str(js['recharge_scenario0']))
  print("------")
  print(str(data['value']['recharge_scenario1']))
  print(str(js['recharge_scenario1']))
  #if str(data['value']['recharge_scenario0']) != str(js['recharge_scenario0']) or str(data['value']['recharge_scenario1']) != str(js['recharge_scenario1']):
    #call("~/apps/cli/bin/metadata-list -v -Q \"{'value.name':'"+dataset_name+"','value.x':"+str(i)+",'value.y':"+str(j)+"}\" > /tmp/res_"+str(i)+"_"+str(j)+".json"
    #call("~/apps/cli/bin/metadata-addupdate -z "+ token +"  -F /tmp/landuse"+str(i)+"_"+str(j)+".json "+ data['uuid'] +";rm /tmp/landuse"+str(i)+"_"+str(j)+".json", shell=True)
